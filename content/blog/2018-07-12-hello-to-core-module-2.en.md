...
Let's talk about the BigClown tools for a while. Around the same time we finished the **{{< shop "USB Dongle" >}}** development, we decided to create a CLI tool that will simplify the workflow with the BigClown firmware in general. The [**BigClown Firmware Tool**]({{< relref "doc/tools/bcf.md" >}}) (**bcf**) was born. It is a simple Python application distributed through PyPI and available to all platforms (it takes one command to set it up on your machine). This tool works like a package manager for the pre-compiled binaries available nt our GitHub, but also allows you to program custom firmware, search in firmware repository, list available serial devices and much more.
